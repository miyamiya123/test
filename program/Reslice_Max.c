...
*/
		...
		/*各時間における極大値を求める*/
			/*jから+-visionまで一様増加、一様減少となっている位置を探す。
			 * jと+-visionでの輝度の差がerror以上なら極大値と見なす。*/
		printf("Calculating\n");
		for(i = 0; i < time; i++ ){
		...
		/*データの補正*/
			/*時間iと時間i+1における極大値の位置が遠かった場合、
			 * ノイズなどでカウントされなかったと見なし、
			 * 次の瞬間にも同じ場所に極大値があるとする。*/
		printf("Revising\n");
		for(i = 0; i < time; i++ ){
...
*/				}
		...
		/*ファイルに結果を書き込む*/
			/*一列目に時間、N列目にはN-1番目の極大値の位置を書き込む*/
		sprintf(write_filename,"%s_particle0.txt",read_filename);
